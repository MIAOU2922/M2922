    ...
    public static class ShaderGenerator
    {
        ...
        static Dictionary<string,ShaderModule[]> GenerateDuplicates(List<ShaderModule> modules)
        {
            ...
            foreach (var duplicateId in duplicates)
            {
                ...
                void ReplaceAutomaticKeywordId(string keyword, Func<int, string, string> replacer)
                {
                    for(int i=1; i < moduleInstances.Length; i++)
                    {
                        ReplaceInModule(moduleInstances[i], keyword, replacer(i, keyword));
                    }
                }
                ...
                void MakeUnique(IEnumerable<string> keywords, Func<string,string> keywordTransformer, Func<int, string, string> replacer)
                {
                    foreach(var k in keywords)
                    {
                        string keyword = keywordTransformer == null ? k : keywordTransformer(k);
                        bool hasSpecifiedIdentifier = false;
                        
                        foreach (IdReplacement idReplacement in idReplacementTypes)
                        {
                            if (keyword.IndexOf(idReplacement.IndexKeyword, StringComparison.OrdinalIgnoreCase) != -1)
                            {
                                ReplaceCustomKeywordId(keyword, idReplacement);
                        ...
                        {
                            ReplaceAutomaticKeywordId(keyword, replacer);
                        }
