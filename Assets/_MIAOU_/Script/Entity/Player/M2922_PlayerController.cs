...
namespace M2922.Entity.Player
{
    //  Classe contrôlant le joueur local, gérant les systèmes de santé, armure, etc.
    public class M2922_PlayerController : M2922_Entity
    {
        ...
        public void TakeDamage(float damage, int attackerId, DamageType damageType)
        {
            ...
            if (HasArmor) damage = ArmorSystem.AbsorbDamage(damage, damageType);
        ...
        // === IArmored (inline) ===
        public float ArmorPoints    => HasArmor ? ArmorSystem.ArmorPoints    : 0f;
        public float MaxArmorPoints => HasArmor ? ArmorSystem.MaxArmorPoints : 0f;
        public void RepairArmor(float amount)   { if (HasArmor) ArmorSystem.RepairArmor(amount); }
        public void RepairArmorFull()           { if (HasArmor) ArmorSystem.RepairFull(); }
        ...
        protected override void Update()
        {
            ...
            // ArmorRegen
            float armorRate = BuffSystem.GetStatMultiplier(StatType.ArmorHealRate);
            if (armorRate > 0f && HasArmor) ArmorSystem.RepairArmor(armorRate * dt);
        ...
        private void ApplyBuffStaticEffects()
        {
            ...
            if (HasArmor)
            {
                float maxApMult = BuffSystem.GetStatMultiplier(StatType.MaxArmor);
                if (maxApMult != 1f) ArmorSystem.SetMaxArmorPoints(ArmorSystem.MaxArmorPoints * maxApMult);
            }
        }
